...
- Network deadline defaults to 500 ms and is clamped to 50–1000 ms. The deadline covers connection/DNS/TLS/response waiting. Connections are not reused; sockets and timers are unreferenced so they do not keep an exiting CLI alive. Failed events enter a bounded local outbox (maximum 8 events, 7-day TTL) and retry only on later real telemetry calls or explicit `flush()`; there is no background timer or service.
...
- `flush()` performs one explicit retry pass over the bounded outbox and awaits the current queue; it still cannot guarantee delivery. Normal process exit may leave events pending for a later real run. At most 32 in-process calls queue; excess calls are dropped.
...
- Once-event dedupe and its generated `event_id` are persisted **before** transmission. Only a 2xx response acknowledges the event; network errors, timeouts, and non-2xx responses keep the same event in the bounded outbox. Retries reuse the same `event_id`, and the collector's primary key makes them idempotent.
...
- Disabling preserves local dedupe identity/once flags but clears pending delivery so revoked consent cannot cause a later resend. To forget all local state, stop all package processes and delete the package's hashed JSON file. Re-enabling after deletion creates a new UUID and a new funnel cohort. Remote deletion must be handled by your collector's policy.
