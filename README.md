...
```sh
npm install @liushiyumathxjtu/telemetry
```
...
For first publication, authenticate on the publishing machine to an account authorized for the `@liushiyumathxjtu` scope, then:

```sh
npm login --registry=https://registry.npmjs.org/
npm whoami --registry=https://registry.npmjs.org/
npm publish --access public
npm view @liushiyumathxjtu/telemetry@0.1.0 version dist.integrity --registry=https://registry.npmjs.org/
```
